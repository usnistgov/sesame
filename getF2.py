...
def getF(sys, v, efn, efp):
    ...
    jny_s   = get_jn(sys, efn, v, s_spN)
    jny_smN = get_jn(sys, efn, v, smN_s)

    jpx_s   = get_jp(sys, efp, v, s_sp1)
    jpx_sm1 = get_jp(sys, efp, v, sm1_s)
    jpy_s   = get_jp(sys, efp, v, s_spN)
    jpy_smN = get_jp(sys, efp, v, smN_s)
    ...
    vec[[3*s for s in sites]] = an
    vec[[3*s+1 for s in sites]] = ap
    vec[[3*s+2 for s in sites]] = av
    ...
    vec[[3*s for s in sites]] = bn
    vec[[3*s+1 for s in sites]] = bp
    vec[[3*s+2 for s in sites]] = bv      
    ...
    jnx_sm1 = get_jn(sys, efn, v, sm1_s)
    jny_s   = get_jn(sys, efn, v, s_spN)
    jny_smN = get_jn(sys, efn, v, smN_s, (sys.dy[0] + sys.dy[-1]) / 2.)

    jpx_sm1 = get_jp(sys, efp, v, sm1_s)
    jpy_s   = get_jp(sys, efp, v, s_spN)
    jpy_smN = get_jp(sys, efp, v, smN_s, (sys.dy[0] + sys.dy[-1]) / 2.)
    ...
    # compute the currents
    jnx_sm1 = get_jn(sys, efn, v, sm1_s)
    jny_s   = get_jn(sys, efn, v, s_spN, (sys.dy[0] + sys.dy[-1]) / 2.)
    jny_smN = get_jn(sys, efn, v, smN_s)

    jpx_sm1 = get_jp(sys, efp, v, sm1_s)
    jpy_s   = get_jp(sys, efp, v, s_spN, (sys.dy[0] + sys.dy[-1]) / 2.)
    jpy_smN = get_jp(sys, efp, v, smN_s)
