...
def defectsF(sys, n, p, rho, r=None):
    ...
    defects = sys.lines_defects
    if len(defects) == 0:
        defects = sys.planes_defects

    for cdx, c in enumerate(defects):
        sites = sys.extra_charge_sites[cdx]

        nextra = sys.nextra[cdx, sites]
        pextra = sys.pextra[cdx, sites]
        _n = n[sites]
        _p = p[sites]

        Se = sys.Seextra[cdx, sites]
        Sh = sys.Shextra[cdx, sites]
        ...
        # extra charge
        if c.defect_type == 'donor':
            rho[sites] += sys.Nextra[cdx, sites] * (1 - f)
        if c.defect_type == 'acceptor':
            rho[sites] -= sys.Nextra[cdx, sites] * f
        if c.defect_type == 'u-center':
            rho[sites] += sys.Nextra[cdx, sites] * (1./2. - f)
...
def defectsJ(sys, n, p, drho_dv, drho_defn=None, drho_defp=None,\
    ...
    for cdx, c in enumerate(defects):
        ...
        drho_dv[sites] += - sys.Nextra[cdx, sites] *\
            (Se**2*_n*nextra + 2*Sh*Se*_p*_n + Sh**2*_p*pextra) / d
        if drho_defn is not None:
            drho_defn[sites] += - sys.Nextra[cdx, sites] *\
                Se*_n * (Se*nextra + Sh*_p) / d
            drho_defp[sites] += sys.Nextra[cdx, sites] *\
